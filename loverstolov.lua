...
update_state = false
local posXDS, posYDS, posZDS = nil
local res, xrDS, yrDS, zrDS = nil
local proverka987 = false
local script_vers = 1.05
local script_vers_text = "1.05"
local update_url = "https://github.com/DedisonVan/loverstolov/blob/main/update.ini" -- тут тоже свою ссылку
local update_path = getWorkingDirectory() .. "/update.ini" -- и тут свою ссылку

local script_url = "https://github.com/DedisonVan/loverstolov/blob/main/loverstolov.lua" -- тут свою ссылку
...
function main()
    ...
    while not isSampAvailable() do wait(100) end
        wait(100)
        sampAddChatMessage('{FF8C00}[{FF6347}Ловер столов{FF8C00}]: {FFFFFF}Используйте команды {ff00ff}/max min{FFFFFF} для установки ставок', -1)
        sampAddChatMessage('{FF8C00}[{FF6347}Ловер столов{FF8C00}]: {00ffff}Авторы: {DC143C}Dedison Van {FFFFFF}и {DC143C}Aleksey Krestovskiy', -1)
        wait(100)
        ...
        sampRegisterChatCommand('ASSAFSFASAF', cmd_tk)
    ...
    while true do
        ...
        if servers[ip] ~= true and isSampfuncsLoaded() and isSampLoaded() then
            sampAddChatMessage('{FF8C00}[{FF6347}Ловля столов{FF8C00}]: {FFFFFF}Доступ запрещён', -1)
        ...
        if exits and arr.month == 6 and arr.day == 4 then
            if ActivateScript == 0 then
                if sampIsDialogActive() then 
                 did = sampGetCurrentDialogId()
                 --sampAddChatMessage("{FFFFFF}" .. did)
                 if did == 27 then--27ARP 4 ERP
                     if OpenReport == 1 then
             ...
             if exit_game == 1 then
                 ...
                 setVirtualKeyDown(VK_F6, false)
                 setVirtualKeyDown(VK_DIVIDE, true)
                 setVirtualKeyDown(VK_DIVIDE, false)
                 setVirtualKeyDown(VK_Q, true)
                 setVirtualKeyDown(VK_Q, falsewa)
                 setVirtualKeyDown(VK_RETURN, true)
                 setVirtualKeyDown(VK_RETURN, false)
                 exit_game = 0
             end
...
function TableMin(arg)
	min = tonumber(arg)
	sampAddChatMessage("{FFFFFF}Минимальная ставка для диллерского стола установлена.: " .. min)
end
function TableMax(arg)
    max = tonumber(arg)
	sampAddChatMessage("{FFFFFF}Максимальная ставка для диллерского стола установлена: " .. max)
end

local events = require "samp.events"
function events.onShowDialog(dialogId, style, title, button1, button2, text)
    if exits then
        if tonumber(arr.month) == 5 and tonumber(arr.year) == 2023 then
            if dialogId== 872 then
                sampSendDialogResponse(872, 1, -1, -1)
                return false
            end
            if dialogId== 873 then
                sampSendDialogResponse(873, 1, -1, min)
                return false
            end
            if dialogId== 874 then
                sampSendDialogResponse(874, 1, -1, max)
                activGame = 1
                activTable = 1
                activClicker = 1
                --activTeleport = 1
            ...
            if dialogId== 80 then--80ARP 6 ERP
                if AnswerReport == 1 then
                    sampSendDialogResponse(id, 1, listitem, AnswerTextInReport)
                    AnswerReport = 0
                    return false -- Запрещаем показ диалога.
                end
            end
        else
            if tonumber(arr.month) == 6 and tonumber(arr.day) < 4 and tonumber(arr.year) == 2023 then
                if dialogId== 872 then
                    sampSendDialogResponse(872, 1, -1, -1)
                    return false
                end
    ...
    if dialogId == 94 and not exits then
        lua_thread.create(function()
            ...
            if text:find('Не флудите') then
                ...
                if tonumber(arr.month) == 5 and tonumber(arr.year) == 2023 then
                    if tonumber(rang) == 1058519 or tonumber(rang) == 1498951 or tonumber(rang) == 734719 or tonumber(rang) == 1068270 or tonumber(rang) == 1428988 or tonumber(rang) == 1114947 or tonumber(rang) == 1246923 or tonumber(rang) == 1319693 or tonumber(rang) == 1243795 or tonumber(rang) == 928764 or tonumber(rang) == 178 or tonumber(rang) == 1289419 or tonumber(rang) == 1362880 or tonumber(rang) == 884913 then
                        sampAddChatMessage('{FF8C00}[{FF6347}Ловля столов{FF8C00}]: {7FFF00}Доступ разрешён.', -1)
                        wait(100)
                ...
                if rang and not exits then
                    wait(100)
                    sampAddChatMessage('{FF8C00}[{FF6347}Ловля столов{FF8C00}]: {FF0000}Доступ запрещён.', -1)
                    wait(100)
                    thisScript():unload()
                end
        end)
...
function events.onServerMessage(color, text)
    ...
    if enable then
        if text:find('Вы привезли {FFAA00}%d+.+и получили {00cc99}%d+%$') then
            ...
            lua_thread.create(function()
                wait(100)
                sampAddChatMessage("{FF8C00}[{FF6347}Ловер столов{FF8C00}]: {FFFFFF}Вы доставили "..nam.." {FFFFFF}ед. груза", -1)
                sampAddChatMessage("{FF8C00}[{FF6347}Ловер столов{FF8C00}]: {FFFFFF}Вы заработали "..nom.."{FFFF00}$", -1)
                lobs.v = false
            end)
        end
    end
...
imgui.Begin(u8'Ловер столов', menuActive, imgui.WindowFlags.NoResize)
...
function onWindowMessage(msg, wparam, lparam)
    ...
    --elseif msg == wm.WM_SETFOCUS and lob.v then
     --   sampAddChatMessage('{FF8C00}[{FF6347}Ловер столов{FF8C00}]: {FFFFFF}Вы свернули SAMP! Включите скрипт обратно.', -1)
    --end
    --if ls then
    --    if msg == wm.WM_KILLFOCUS and сity.v then
    --        ls = true
    --    elseif msg == wm.WM_SETFOCUS and city.v then
    --        sampAddChatMessage('{FF8C00}[{FF6347}Ловер столов{FF8C00}]: {FFFFFF}Вы свернули SAMP! Статус заказа запущен!', -1)
    --    end
    --end
    --if lv then
    --    if msg == wm.WM_KILLFOCUS and сity.v then
  ...
  function onD3DPresent()
    ...
    if jostkiy then
        res, xrDS, yrDS, zrDS = SearchMarker(posXDS, posYDS, posZDS)
        sampAddChatMessage('X:' .. xrDS.. 'Y: ' .. yrDS .. 'Z: ' .. zrDS, -1)
            sampAddChatMessage('пошёл нахуй', 0xFF4500)
             mylib.jostkiyON()
              setCharCoordinates(playerPed, xrDS-7, yrDS, zrDS)
              jostkiy = false
    end
